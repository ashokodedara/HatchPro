				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("MpId") === oObject.MpId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].MpId;
										} else {
											sCurrentId = aItemsNotInTheList[0].MpId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("MYPROJECTSSet", {
											MpId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("MpId"),
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "MYPROJECTSSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'MYPROJECTSSet (0)'");
							},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all MYPROJECTSSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "MYPROJECTSSet (" + iExpectedLength + ")"}),
									success : function () {
