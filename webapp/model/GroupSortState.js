sap.ui.define([
	...
	return BaseObject.extend("webapp.model.GroupSortState", {
		 ...
		 * @alias webapp.model.GroupSortState
		 */
		...
		/**
		 * Sorts by MpId, or by ActualData
		 *
		...
		/**
		 * Groups by ActualData, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "ActualData") {
				...
				this._oViewModel.setProperty("/sortBy", "ActualData");

				aSorters.push(
					new Sorter("ActualData", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "MpId");
			}
