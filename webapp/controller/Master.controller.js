...
sap.ui.define([
	...
	"webapp/model/grouper",
	"webapp/model/GroupSortState",
	"sap/ui/model/Sorter",
	"jquery.sap.global"
	...
	return BaseController.extend("webapp.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("MpId", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("webapp.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("ActualData", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("ActualData", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
		...
		_onMasterMatched: function() {
			this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					var sObjectId = mParams.firstListitem.getBindingContext().getProperty("MpId");
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("MpId")
			}, bReplace);
		},
